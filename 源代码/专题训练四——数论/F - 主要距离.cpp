...
bool low_v[N],v[N];
vector<int> prime;
void qjs(ll a,ll b){//区间筛 
	for(ll i=0;i*i<b;i++)low_v[i]=1; 
	for(int i=0;i<=b-a;i++)v[i]=1;
	low_v[1]=0;//1不为素数 
	if(a==1)v[0]=0;//因为L最小为1，但1 不是素数需要特判。 
	for(ll i=2;i*i<b;i++){
		if(low_v[i]){
			for(ll j=2*i;j*j<=b;j+=i)low_v[j]=0;//筛[2，sqrtb)； 
			for(ll j=max(2LL,(a+i-1)/i)*i;j<=b;j+=i)v[j-a]=0;//筛[a,b) 
		}
	}
}
...
//		for(int i=0;i<prime.size();i++)cout<<prime[i]<<' ';
		if (prime.size() < 2)//如果区间的素数小于2则证明区间内无相邻 
		{
